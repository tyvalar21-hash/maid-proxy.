...
// Хранилище диалогов (только для admin и vip)
const chatHistory = {};
const chatSummaries = {};
const chatBlocks = {};

// Хранилище фактов об игроках
const playerFacts = {};

// Ожидание подтверждения факта
...
function extractFactFromMessage(message, playerId, playerRole) {
    ...
    // Псевдоним (если имя уже есть)
    if (nameMatch && playerFacts[playerId].name) {
        return { type: "nickname", value: nameMatch[1] };
    }
    
    // Возраст (только если ещё не задан)
    ...
    // Откуда (только если ещё не задано)
...
// Сборка фактов в строку для промпта
...
app.post("/chat", async (req, res) => {
    ...
    // Проверка ожидающего подтверждения
    if (pendingConfirmations[playerId] && saveMemory) {
        const pending = pendingConfirmations[playerId];
        const msgLower = message.toLowerCase();
        
        if (msgLower === "да" || msgLower === "yes" || msgLower === "подтверждаю" || msgLower === "верно" || msgLower === "правильно" || msgLower === "ага" || msgLower === "ок" || msgLower === "хорошо") {
        ...
        if (msgLower === "нет" || msgLower === "no" || msgLower === "неверно" || msgLower === "неправильно" || msgLower === "отмена" || msgLower === "не") {
            delete pendingConfirmations[playerId];
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (data.choices && data.choices[0]) {
                ...
                if (!isCommand && !isTranslation && !translateForGuest && !translateFromGuest && !isSearchQuery && saveMemory) {
